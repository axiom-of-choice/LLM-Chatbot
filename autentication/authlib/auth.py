...
def _auth(sidebar=True, show_msgs=True):
    ...
    if auth_state().user == None:
        ...
        if user_in_cookie:
            ctx = {
                "fields": "*",
                "conds": f'username="{user_in_cookie[const.USERNAME]}"',
            }
    ...
    if auth_state().user != None:
        return {
            "user": auth_state().user[const.USERNAME],
            "cf": auth_state().user[const.CF],
            "su": auth_state().user[const.SU],
        }
    else:
        return None
...
def _list_users():
    ...
    ctx = {"fields": "username, password, su, cf"}
    data = store.query(context=ctx)
    if data:
        st.table(data)
    else:
...
def _edit_user():
    ...
    if username:
        ctx = {
            "fields": "username, password, su, cf",
            "conds": f'username="{username}"',
        }
...
def get_CF():
    data = store.query(
        context={
            "fields": "username, CF",
            "conds": f'username="{auth_state().user[const.USERNAME]}"',
        }
    )
    return data[0][const.CF] if data else None
...
def admin():
    ...
    if st.checkbox(
        "I accept responsibility and understand this mode can be used to initialise and make changes to the authentication database"
    ):
        from authlib.repo.storage_factory import StorageFactory
